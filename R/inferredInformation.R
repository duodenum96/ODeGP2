detectTimePeriod <- function(d, res, kernel, K_inv) {
  ...
  if (kernel == "stationary") {
    Q <- stationary.Q(res$par)
  } else if (kernel == "nonstationary") {
    Q <- nonstationary.Q(res$par)
  } else {
    Q <- 1
  }
...
modelSelection <- function(statistic, threshold) {
  ...
  cat(
    "If k trajectories from the same dataset are being analyzed simultaneously, a Bonferroni-type multiple-hypothesis correction can be applied by multiplying each Bayes factor, obtained independently, with (1-0.5^(1/k))/(0.5^(1/k)).\n"
  )
