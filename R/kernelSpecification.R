...
stationaryFunc <- function(x1, x2, params) {
  ...
  for (i in 1:Q) {
    decay <- exp(
      -2 * square(pi) * square(params[(3 * i) - 1]) * square(x1 - x2)
    )
  ...
  if (x1 == x2) {
    sum <- sum + square(params[length(params)])
  }
  return(sum)
}
...
nonstationaryFunc <- function(x1, x2, params) {
  ...
  for (i in 1:Q) {
    ...
    if (hyp$l1 < 0) {
      cat("l1 is", hyp$l1, "and l2 is", hyp$l2, "\n")
    }
...
nonstationaryBounds <- function(d, Q) {
  ...
  single_lower <- c(
    m$Y_step,
    0,
    0, #w func lower bounds
    m$timestep,
    0,
    0, #l func lower bounds
    m$X_range / 2,
    2 * m$timestep,
    0
  ) #p func lower bounds
  single_upper <- c(
    m$Y_range,
    m$Y_step,
    m$X_range, #w func upper bounds
    m$X_range,
    m$timestep,
    m$X_range, #l func upper bounds
    m$X_range,
    m$X_range / 2,
    m$X_range
  ) #p func upper bounds
