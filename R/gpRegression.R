...
covarianceMatrix <- function(params, X1, X2, S, kernel) {
  ...
  for (i in 1:m) {
    for (j in 1:n) {
      ...
      if (i == j & !is.null(S)) {
        cov_mat[i, i] <- cov_mat[i, i] + square(S[i])
      }
    }
  }
...
getPosterior <- function(dataset, res, kernel, plotting = FALSE, Q = NULL) {
  ...
  for (i in 1:m) {
    confidence_bounds[i] <- 2 * sqrt(cov_x[i, i])
  }
...
plotPriorSamples <- function(dataset, kernel, Q = NULL) {
  ...
  for (i in 1:m) {
    prior_bounds[i] <- 2 * sqrt(K_prior[i, i])
  }
...
plotPosteriorSamples <- function(dataset, test, mu_x, cov_x, bounds_x) {
  ...
  names(pt_df) <- c("X", "Y", "S")
  post <- post +
    geom_point(data = pt_df, aes(x = X, y = Y), size = 2, colour = "black") # +
